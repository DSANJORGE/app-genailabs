...
final _manager = AdminMe('m', 'm@x', 'Lider', 'manager', {
  'analytics_view',
  'personas_view',
}, const []);
...
void main() {
  testWidgets('lists recent comments and open question reports', (
    tester,
  ) async {
    ...
    expect(
      find.byType(TestuThread),
      findsNothing,
      reason: 'no thread open yet',
    );
  });

  testWidgets('a row opens its thread and a reply posts through comment.json', (
    tester,
  ) async {
  ...
  testWidgets('empty scope reads as an empty state, not a blank card', (
    tester,
  ) async {
    await _pump(tester, recent: {'ok': true, 'recent': [], 'flags': []});
    expect(find.byType(EmptyState), findsOneWidget);
    expect(
      find.text('Nobody has commented or reported a question yet.'),
      findsOneWidget,
    );
  });
