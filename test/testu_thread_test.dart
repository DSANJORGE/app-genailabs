...
void main() {
  testWidgets('renders live rows with names, badges, initials and reply counts', (
    tester,
  ) async {
    await _pump(tester);
    ...
    expect(
      find.byType(Image),
      findsNothing,
      reason: 'live rows draw initials, no asset photo',
    );
  ...
  testWidgets(
    'sending posts to comment.json, reloads and clears the composer',
    (tester) async {
  ...
  testWidgets(
    'a reaction posts to react.json and resyncs the row with the server counts',
    (tester) async {
      final http = await _pump(tester);
  ...
  testWidgets('a failed load shows the error and Retry refetches', (
    tester,
  ) async {
  ...
  testWidgets('Tab out of the composer field is not trapped by a focus scope', (
    tester,
  ) async {
    final http = FakeEmeHttp();
    ...
    expect(
      composerFocus,
      isNot(sibling),
      reason: 'the composer field should hold focus first',
    );
    ...
    for (
      var i = 0;
      i < 4 && FocusManager.instance.primaryFocus != sibling;
      i++
    ) {
  ...
  testWidgets('the demo entry still shows the mock thread', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: testuTheme(),
        home: const Scaffold(
          body: SingleChildScrollView(child: SocialThreadEntry()),
        ),
      ),
    );
    await tester.pumpAndSettle();
    ...
    expect(
      find.textContaining('Conversations on this question · 4'),
      findsOneWidget,
    );
  });
}
